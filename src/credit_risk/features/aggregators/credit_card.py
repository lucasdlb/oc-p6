"""Credit card balance table aggregator."""
...
class CreditCardAggregator(TableAggregator):
    """Aggregator for credit_card_balance table."""
    ...
    def load_link(cls) -> LazyFrame | None:
        """Credit card has SK_ID_CURR directly, no link needed."""
        return None
    ...
    def aggregate(self, df: LazyFrame, method: str = "default") -> LazyFrame:
        ...
        Args:
            df: credit_card_balance LazyFrame
        ...
        agg_exprs = _get_agg_exprs(agg_cols, numeric_cols, "cc_", methods)
        agg_exprs.append(pl.col("SK_ID_CURR").count().alias("cc_n_records"))
