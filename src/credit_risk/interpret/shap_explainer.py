...
from credit_risk.config.settings import InterpretConfig


class ShapExplainer:
    def __init__(self, config: InterpretConfig | None = None):
        self.config = config or InterpretConfig()
    ...
    def fit(self, model: BaseEstimator, X_background: np.ndarray) -> "ShapExplainer":
        background = shap.sample(X_background, self.config.shap_background_samples, random_state=42)
    ...
    def global_importance(
        ...
        n = n_samples or self.config.shap_n_samples
