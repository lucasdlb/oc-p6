...
class ManyModelOptunaTuner:
    ...
    def optimize_sequential(
        ...
        for model_name in model_names:
            ...
            factory = get_factory(model_name, x_transform=self._config.x_transform)
    ...
    def get_best_model(
        ...
        X: np.ndarray,
        y: np.ndarray,
    ) -> tuple[BaseEstimator, str, dict[str, Any]]:
        """Get best model across all optimized models.

        Note: This trains on full data, not CV. This is the final model fit.
        ...
        Returns:
            Tuple of (trained model, model name, all results)
        """
        if not self._results:
            raise RuntimeError("Must call optimize_sequential() first")

        best_model_name = max(self._results, key=lambda k: self._results[k]["best_value"])
        best_params = self._results[best_model_name]["best_params"]

        logger.info(f"Training final model for {best_model_name} on full data")

        factory = get_factory(best_model_name, x_transform=self._config.x_transform)
        model = factory(**best_params).build_model_pipeline()
        model.fit(X, y)

        return model, best_model_name, self._results
